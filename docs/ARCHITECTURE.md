...
command errored) becomes an `Error` finding for the checks that needed it; the
rest still run and the errored check is excluded from the score. The exception is
a package-manager check whose tool is simply absent (exit 127 — e.g. `apt-get` on
RHEL, `dnf` on Debian): that is reported `Skipped` (not applicable), not an error.
...
| `checks/*.rs`    | The 45 checks, grouped by domain; each is a pure `evaluate(output) → Outcome`. `Check::privileged()` marks `sudo -n` checks, run only on opted-in targets (else `Skipped`). |
