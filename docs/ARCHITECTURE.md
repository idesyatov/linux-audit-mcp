...
| `checks/*.rs`    | The 38 checks, grouped by domain; each is a pure `evaluate(output) → Outcome`. `Check::privileged()` marks `sudo -n` checks, run only on opted-in targets (else `Skipped`). |
