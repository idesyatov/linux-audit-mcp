...
fd_crit_pct = 95
conntrack_warn_pct = 80   # conntrack table fill as % of nf_conntrack_max (health-conntrack)
conntrack_crit_pct = 90
pid_warn_pct = 80         # kernel tasks as % of pid_max (health-pids)
pid_crit_pct = 90
tcp_warn_pct = 80         # worst of TCP mem/orphan/TIME_WAIT as % of its ceiling (health-tcp-mem)
tcp_crit_pct = 90
...
| accounts  | `accounts-duplicate-uid`       | Medium   | two or more accounts share a UID           |
...
| kernel    | `kernel-ptrace-scope`          | Low      | `yama.ptrace_scope` = 0 (unrestricted)     |
| kernel    | `kernel-dmesg-restrict`        | Low      | `dmesg_restrict` ≠ 1                        |
| kernel    | `kernel-kptr-restrict`         | Low      | `kptr_restrict` not 1/2                     |
...
| kernel    | `kernel-world-writable-dirs` 🔑 | Medium  | a world-writable directory **without the sticky bit** exists (any user can delete/replace another's files; `/tmp` etc. are sticky and excluded) |
